package com.example.socialmedia.notification.config;
...
public class CorrelationIdFilter extends OncePerRequestFilter {
    ...
    private static String correlationId(String candidate) {
        return candidate != null && SAFE_VALUE.matcher(candidate.trim()).matches()
    ...
    private static final class CorrelationRequest extends HttpServletRequestWrapper {
        ...
        public Enumeration<String> getHeaders(String name) {
            return HEADER_NAME.equalsIgnoreCase(name)
                    ? Collections.enumeration(Collections.singleton(correlationId)) : super.getHeaders(name);
        }
    }
}
