...
public class GatewaySecurityConfig {
    ...
    @Bean
    SecurityFilterChain gatewaySecurityFilterChain(HttpSecurity http, Cache<String, Bucket> rateLimitBuckets)
            throws Exception {
        ...
        http.csrf(csrf -> csrf.disable())
                ...
                .authorizeHttpRequests(authorize -> authorize
                        ...
                        .requestMatchers(HttpMethod.GET, "/api/v1/profiles/**", "/api/v1/posts/*",
                                "/api/v1/users/*/posts").permitAll()
                        .anyRequest().authenticated())
                .oauth2ResourceServer(resourceServer -> resourceServer
                        .jwt(jwt -> jwt.jwtAuthenticationConverter(converter))
                        .authenticationEntryPoint((request, response, exception) ->
                                writeProblem(response, request, 401, "Unauthorized")))
                .exceptionHandling(exceptions -> exceptions
                        .accessDeniedHandler((request, response, exception) ->
                                writeProblem(response, request, 403, "Forbidden")))
                .addFilterAfter(new RateLimitConfig.RateLimitFilter(rateLimitBuckets),
    ...
    private static void writeProblem(HttpServletResponse response, HttpServletRequest request,
            int status, String title) throws IOException {
