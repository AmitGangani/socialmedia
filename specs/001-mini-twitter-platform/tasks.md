...
- [X] T092 [P] [US5] Implement Notification with FOLLOW/REPLY invariants, UUIDv7 identity, source-event uniqueness, recipient keyset ordering, and targeted Lombok persistence annotations in `services/notification-service/src/main/java/com/example/socialmedia/notification/domain/Notification.java`
- [X] T093 [P] [US5] Implement the composite-key ProcessedEvent model for `notification-service-v1` event identities in `services/notification-service/src/main/java/com/example/socialmedia/notification/domain/ProcessedEvent.java`
...
- [X] T095 [P] [US5] Implement Notification's local opaque `(eventTime,id)` cursor codec with malformed/version/page-size rejection in `services/notification-service/src/main/java/com/example/socialmedia/notification/application/CursorCodec.java`
- [X] T096 [US5] Implement one-transaction notification plus processed-identity commits, self-notification no-ops, duplicate exits, and newest-first retrieval in `services/notification-service/src/main/java/com/example/socialmedia/notification/application/NotificationService.java`
- [X] T097 [P] [US5] Configure initial delivery plus two one-second Kafka retries and Notification-specific DLT publication with sanitized failure metadata in `services/notification-service/src/main/java/com/example/socialmedia/notification/config/KafkaFailureConfig.java`
- [X] T098 [US5] Consume follow-created and reply-bearing post-published facts, explicitly no-op irrelevant known events, and reject unknown types to DLT in `services/notification-service/src/main/java/com/example/socialmedia/notification/integration/NotificationEventConsumer.java`
- [X] T099 [US5] Expose only JWT-subject-owned newest-first notification retrieval with no target-user input in `services/notification-service/src/main/java/com/example/socialmedia/notification/api/NotificationController.java`
- [X] T100 [US5] Configure Notification event deserialization, consumer groups, UUIDv7 generation, and transaction boundaries in `services/notification-service/src/main/java/com/example/socialmedia/notification/NotificationApplication.java`
- [X] T101 [US5] Add recipient notification retrieval, cross-user denial, consistency polling, DLT inspection, and unchanged-event replay requests/commands to `http/socialmedia.http`
- [X] T102 [US5] Execute and record the 10-follow/10-reply timing table, recipient isolation, DLT handling, and duplicate replay evidence under Scenarios 4, 4A, 6, and 8 in `specs/001-mini-twitter-platform/quickstart.md`
